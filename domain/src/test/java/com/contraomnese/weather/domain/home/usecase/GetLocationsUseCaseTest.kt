...
class GetLocationsUseCaseTest {
    ...
    @Test
    fun `given repository returns success result when invoke is called then returns locations`() = runTest {
        coEvery { repository.getLocationsBy(request) } returns Result.success(expectedData)
        ...
        coVerify(exactly = 1) { repository.getLocationsBy(request) }
        confirmVerified(repository)
    }

    @Test
    fun `given repository returns failure result when invoke is called then throws exception`() = runTest {
        coEvery { repository.getLocationsBy(request) } returns Result.failure(expectedException)
        val actualResult = useCase(request)
