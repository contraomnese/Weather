...
class AddFavoriteUseCaseTest {
    ...
    @BeforeEach
    fun setUp() {
        useCase = AddFavoriteUseCaseImpl(repository)
    }
    ...
    fun `should return success result with unit when invoke is called`() = runTest {
        ...
        coEvery { repository.addFavorite(favoriteLocationId) } returns Result.success(expectedData)
        val actualResult = useCase(favoriteLocationId)

        val actualData = actualResult.assertIsSuccess()
        assertEquals(expectedData, actualData)
        coVerify(exactly = 1) { repository.addFavorite(favoriteLocationId) }
    ...
    fun `should return failure result with exception invoke is called`() = runTest {
        ...
        coEvery { repository.addFavorite(favoriteLocationId) } returns Result.failure(expectedException)
        val actualResult = useCase(favoriteLocationId)
