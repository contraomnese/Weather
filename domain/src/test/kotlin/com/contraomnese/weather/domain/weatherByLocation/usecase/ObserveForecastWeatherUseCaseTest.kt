...
class ObserveForecastWeatherUseCaseTest {
    ...
    private lateinit var useCase: ObserveSingleForecastUseCase
    private val repository = mockk<ForecastRepository>()

    @BeforeEach
    fun setUp() {
        useCase = ObserveSingleForecastUseCaseImpl(repository)
    }
    ...
    fun `should return flow with correct item when invoke is called`() = runTest {
        ...
        coEvery { repository.observeSingleForecast(locationId) } returns flowOf(expectedData)

        val actualData = useCase(locationId).first()

        assertNotNull(actualData)
        assertEquals(expectedData, actualData)
        coVerify(exactly = 1) { repository.observeSingleForecast(locationId) }
    ...
    fun `should return flow with correct multiple items when invoke is called`() = runTest {
        ...
        coEvery { repository.observeSingleForecast(locationId) } returns flow {
            emit(firstItem)
            emit(secondItem)
        }
